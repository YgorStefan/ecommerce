...
describe('CartService', () => {
  ...
  describe('addItem', () => {
    it('deve lançar NotFoundException se o produto não existir', async () => {
      ...
      await expect(service.addItem('u1', 'p1', 1)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('deve lançar BadRequestException se o estoque for insuficiente', async () => {
      ...
      await expect(service.addItem('u1', 'p1', 5)).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('deve somar a quantidade se o item já existir no carrinho', async () => {
      ...
      expect(mockCartItemsRepo.update).toHaveBeenCalledWith('ci1', {
        quantity: 5,
      });
    });
    ...
    it('deve lançar BadRequestException se a soma exceder o estoque', async () => {
      ...
      await expect(service.addItem('u1', 'p1', 3)).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe('updateItem', () => {
    it('deve lançar NotFoundException se o carrinho não existir', async () => {
      ...
      await expect(service.updateItem('u1', 'ci1', 2)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('deve remover o item quando a quantidade for zero', async () => {
      ...
      mockCartItemsRepo.findOne.mockResolvedValue({
        id: 'ci1',
        product: { stock: 10 },
      });
    ...
    it('deve lançar BadRequestException se a nova quantidade exceder o estoque', async () => {
      ...
      await expect(service.updateItem('u1', 'ci1', 5)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
